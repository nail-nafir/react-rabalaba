-- Drop functions created in migration 08 to recreate them with the p_is_blocked parameter
drop function if exists public.admin_create_user(text, text, text, boolean, boolean, timestamptz);
drop function if exists public.admin_update_user(uuid, text, boolean, boolean, timestamptz);

-- Re-create admin_create_user with p_trial_expires_at & p_is_blocked
...
begin
  ...
  insert into public.profiles (user_id, tier, is_admin, is_owner, trial_expires_at, is_blocked, updated_at)
  values (
    v_user_id,
    p_tier,
    p_is_admin,
...
-- Re-create admin_update_user with p_trial_expires_at & p_is_blocked
