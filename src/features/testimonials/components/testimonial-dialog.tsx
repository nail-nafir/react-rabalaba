...
function statusVariant(status: TestimonialStatus) {
  if (status === "approved") return "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" as const;
  if (status === "rejected") return "bg-destructive/10 text-destructive border-destructive/20" as const;
...
export function TestimonialDialog({
  ...
  }, [form, isError, isLoading, open, submission]);
  ...
  const onSubmit = async (values: TestimonialFormValues) => {
    try {
      ...
      toast.success(
        t(
          "testimonials.form.save_success",
          "Ulasan tersimpan dan dikirim untuk ditinjau.",
        ),
      );
      onOpenChange(false);
    } catch {
      toast.error(
        t(
          "testimonials.form.save_error",
          "Ulasan belum berhasil disimpan. Coba lagi.",
        ),
      );
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(
        t("testimonials.form.delete_success", "Ulasan berhasil dihapus."),
      );
      onOpenChange(false);
    } catch {
      toast.error(
        t(
          "testimonials.form.delete_error",
          "Ulasan belum berhasil dihapus. Coba lagi.",
        ),
      );
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-md border border-border text-foreground">
        <DialogHeader>
          ...
          >
            {submission && (
              <Card size="sm">
                <CardHeader>
                  ...
                  <CardAction>
                    <Badge variant="outline" className={statusVariant(submission.status)}>
                      {submission.status === "approved"
            ...
            <DialogFooter>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>
                        {t(
                          "testimonials.form.delete_title",
                          "Hapus ulasan?",
                        )}
                      </AlertDialogTitle>
                      <AlertDialogDescription>
                    ...
                    <AlertDialogFooter>
                      ...
                      >
                        ...
                        {t("testimonials.form.confirm_delete", "Hapus")}
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
              ...
              >
                ...
                {isSaving
                  ? t("testimonials.form.saving", "Menyimpan...")
                  : t("testimonials.form.submit", "Kirim")}
              </Button>
            </DialogFooter>
          </form>
        )}
