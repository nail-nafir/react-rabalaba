...
import {
  ...
  type ValidatePasswordFormValues,
} from "../schemas/validate-password-schema";
...
function ValidatePasswordDialogContent({
  ...
  const form = useForm<ValidatePasswordFormValues>({
  ...
  const onSubmit = async (data: ValidatePasswordFormValues) => {
    if (!user?.email) {
