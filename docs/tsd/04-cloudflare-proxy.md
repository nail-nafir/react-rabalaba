...
> 🇮🇩 Tier proxy JSON caching di Cloudflare Pages Functions: 4 route + engine cache fresh/stale/error + Yahoo crumb.
> 🇺🇸 JSON caching proxy tier on Cloudflare Pages Functions: 4 routes + fresh/stale/error cache engine + Yahoo crumb.

---

## TL;DR

🇮🇩 `functions/api/` adalah tier proxy JSON caching. Satu engine (`_shared/proxy.ts`) + 4 route (`coingecko`/`binance`/`yahoo`/`fng`). Cache pakai Cloudflare `caches.default` (per-colo) + coalescing in-isolate + SWR on 429/5xx + error caching. Yahoo `/quoteSummary`+`/screener` butuh crumb/cookie auth (proxy minta sendiri). **Browser** sekarang direct CoinGecko/Binance (IP visitor); proxy tetap dipake cron.

🇺🇸 `functions/api/` is a JSON caching proxy tier. One engine (`_shared/proxy.ts`) + 4 routes. Cache uses Cloudflare `caches.default` (per-colo) + in-isolate coalescing + SWR on 429/5xx + error caching. Yahoo `/quoteSummary`+`/screener` need crumb/cookie auth (the proxy mints it). The **browser** now calls CoinGecko/Binance direct (visitor IP); the proxy is still used by the cron.
...
| `fng/[[path]].ts:7` | `api.alternative.me` | 12h / 2d / 60s | `[429, 500-599]` | — | Fear & Greed |

---

## 🧭 Browser vs cron path

| Caller | CoinGecko `/global` | Binance derivatives | Yahoo | F&G |
|---|---|---|---|---|
| **Browser** | **direct** `api.coingecko.com` (IP visitor) | **direct** `fapi.binance.com` (IP visitor) | proxy `/api/yahoo` (crumb gating) | proxy `/api/fng` |
...
> Vite dev parity: `vite.config.ts` proxy `/api/yahoo`+`/api/fng` (+ custom `yahooCrumbDevPlugin` buat crumb dev-only). Entry proxy CoinGecko/Binance dihapus dari vite config karena browser direct.
